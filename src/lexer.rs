    ...
    pub fn tokenize(&mut self) -> Vec<Token> {
        ...
        while self.position < self.input.len() {
            ...
            match char {
                ...
                '=' => {
                    if self.peek_next() == '=' { self.position += 2; tokens.push(Token::EqualEqual); } 
                    else { tokens.push(Token::Equal); self.position += 1; }
                }
                '!' => {
                ...
                '<' => { 
                    if self.peek_next() == '=' { self.position += 2; tokens.push(Token::LessEqual); }
                    else { tokens.push(Token::Less); self.position += 1; } 
                }
                '&' => {
                    if self.peek_next() == '&' { self.position += 2; tokens.push(Token::AmperAmper); }
                    else { tokens.push(Token::Unknown('&')); self.position += 1; }
                }
                '|' => {
                    if self.peek_next() == '|' { self.position += 2; tokens.push(Token::PipePipe); }
                    else { tokens.push(Token::Unknown('|')); self.position += 1; }
                }
