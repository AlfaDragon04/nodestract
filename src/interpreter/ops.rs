    ...
    pub fn eval_binary_op(&self, left: Value, operator: &str, right: Value) -> Value {
        ...
        match (l_res, r_res) {
            ...
            (Value::Integer(a), Value::Integer(b)) => match operator {
                ...
                "*" => Value::Integer(a * b),
                "/" => if b == 0 { println!("MATH ERROR: Division by zero."); Value::Null } else { Value::Integer(a / b) },
            ...
            (Value::Float(a), Value::Float(b)) => match operator {
                ...
                "*" => Value::Float(a * b),
                "/" => if b == 0.0 { println!("MATH ERROR: Division by zero."); Value::Null } else { Value::Float(a / b) },
                ">" => Value::Boolean(a > b), "<" => Value::Boolean(a < b),
                ">=" => Value::Boolean(a >= b), "<=" => Value::Boolean(a <= b),
                "==" => Value::Boolean(a == b), "!=" => Value::Boolean(a != b),
                _ => Value::Null,
